...
fn main() {
    ...
    trigger(resolver.clone(), print_link);
    trigger(resolver.clone(), print_all);
    trigger(resolver.clone(), print_rev);
    // trigger(resolver.clone(), print_all);
    // trigger(resolver.clone(), print_rev);
    // let method = Method::from_context(&resolver);
    // print_all("hello".to_string(), method, Args(vec!["0".to_string(), "1".to_string()]));
}
